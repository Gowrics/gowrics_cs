...
ALTER TABLE CONTAINS  DROP CONSTRAINT SYS_C008390 ; --then insert

alter table CONTAINS add constraint APNAME_FK foreign key(AP_NAME) references AIRPORT(AP_NAME) ON DELETE CASCADE; --after  insert then try to add cnst 


alter table CONTAINS add constraint APNAME_FK foreign key(AP_NAME) references AIRPORT(AP_NAME) ON DELETE CASCADE;



select * from user_constraints where table_name='CONTAINS';
...
ALTER TABLE EMPLOYEE1  DROP CONSTRAINT SYS_C008404 ;

ALTER TABLE EMPLOYEE1 
ADD CONSTRAINT  fk_add FOREIGN KEY(AP_NAME) REFERENCES AIRPORT(AP_NAME) ON DELETE CASCADE;
...
WHERE CONSTRAINT_NAME='SYS_C008390';
...
select * from user_constraints
where constraint_name='SYS_C008403';
...
INSERT INTO TICKET1(TICKET_NUMBER, SOURCE, DESTINATION, DATE_OF_BOOKING,
DATE_OF_CANCELLATION, DATE_OF_TRAVEL, SEATNO, CLASS, PID, PASSPORTNO)
VALUES(8456722229921,'JFK','BOM','11-JUN-16','10-DEC-16','20-DEC-
...
select * FROM AIRPORT;
select * FROM AIRLINE;
select * FROM CONTAINS; --SYS_C008390
select * FROM FLIGHT;
select * FROM PASSENGER1;
select * FROM PASSENGER2;
select * FROM EMPLOYEE1; -- SYS_C008404--SYS_C008382
select * FROM EMPLOYEE2;
select * FROM SERVES;  --  SYS_C008412  --SYS_C008403
select * FROM TICKET1;
select * FROM TICKET2;
select * FROM TICKET3;
...
SELECT    al.AL_NAME,fl.FLIGHT_CODE,p2.FNAME,p2.LNAME,p2.PASSPORTNO,
          t.CLASS,t.DATE_OF_TRAVEL,t.DESTINATION,t.SOURCE,t.SEATNO,
          t.TICKET_NUMBER 
...
select
al.AL_NAME,fl.FLIGHT_CODE,p2.FNAME,p2.LNAME,p2.PASSPORTNO,t.CLASS,t.DATE_OF_TRAVEL,t.DESTINATION,t.SOURCE,t.SEATNO,t.TICKET_NUMBER from Airline al,Flight fl,PASSENGER1 p1,PASSENGER2 p2
...
SELECT DISTINCT F.FLIGHT_CODE ,F.ARRIVAL,F.DEPARTURE,
F.SOURCE,F.DESTINATION,F.STATUS,F.FLIGHTTYPE
FROM FLIGHT F
...
IF updating then
INSERT INTO DELAYED_FLIGHTS (FLIGHT_CODE, SOURCE, DESTINATION , AIRLINEID ,REPORT_TIME)
VALUES(:old.FLIGHT_CODE,:old.SOURCE,:old.DESTINATION,:old.AIRLINEID,sysdate);
end if ;
END;
/
...
update flight 
set status ='Delayed'
where flight_code ='AI2014';


delete DELAYED_FLIGHTS where FLIGHT_CODE='QR129';
delete flight where FLIGHT_CODE='QR129';
