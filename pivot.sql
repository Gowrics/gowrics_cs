...
select * from 
(select hire_date,department_id,salary from employees )
pivot (sum(salary)
...
select * from 
(select hire_date,department_id,salary from employees WHERE TO_CHAR(hire_date,'yy')>=01)
pivot (sum(salary)
for department_id in (10,20,30)
);
...
select * FROM
(select department_id,salary from employees)
pivot (sum(salary),count(*)cnt
for department_id in (10 dept_sal_10,20 dept_sal_20,30 dept_sal_40));
...
SELECT (SELECT SUM(salary) FROM employees WHERE department_id=10)"10",
       (SELECT SUM(salary) FROM employees WHERE department_id=20)"20",
       (SELECT SUM(salary) FROM employees WHERE department_id=30)"30"
FROM dual);
...
SELECT * FROM
(
SELECT department_id,salary,employee_id FROM employees
)
UNPIVOT (column_value FOR column_name IN(department_id,salary,employee_id)) ;


--cretae a view pivot resuly
CREATE OR REPLACE VIEW sums_sal_vw
AS
(
select * from 
...
--convert into unpivot
SELECT * FROM sums_sal_vw
UNPIVOT (col_value FOR col_name IN(10,20,30));

--error accur
--bcoz the column shuld be same data type
--(first_name,employee_id ,job_id) emp id is diff data type
SELECT * FROM
(
SELECT first_name,employee_id ,job_id FROM employees
)
UNPIVOT (column_value FOR column_name IN(first_name,job_id)) ;

--date should be convert to char
SELECT * FROM
(
SELECT first_name,to_char(hire_date,'dd/mon/yy') dates,job_id FROM employees
)
UNPIVOT (col_value FOR column_name IN(first_name,job_id,dates));


SELECT * FROM
(
SELECT TO_CHAR(department_id) deptno,first_name,TO_CHAR(employee_id)empno FROM employees
)
UNPIVOT (col_val FOR col_name IN (deptno,first_name,empno));
...
select * from employees
pivot( count(*) for  job_id in ('AC_ACCOUNT','AC_MGR','AD_ASST','AD_PRES','FI_ACCOUNT'))
order by employee_id;
;   
